...
const getItemStyle = (isDragging, draggableStyle) => ({
  // some basic styles to make the products look a bit nicer
  userSelect: 'none',
  padding: '1px',
  margin: `0 0 1px 0`,
...
export default class ReorderProducts extends Component {
  constructor(props) {
    super(props);
    this.state = {
      products: []
  ...
  onDragEnd (result) {
    ...
    products.map((item, index) => (
        axios.patch('http://localhost:8000/update-product-order/', {id:item.id, order:item.order}).then((response) => {
            console.log('Success');
        })
        .catch(function (error) {
            console.log(error);
                          ...
                          <div className="row">
                              <div className="col-xs-10" style={{ color:'#333', fontSize:'16px' }}>
                                  ...
                                  <div style={{ display:'inline-block', position:'relative', bottom:'1px' }}>
                                      <div>{item.title}</div>
                                      <small style={{ color:'#636b6f' }}>£{item.price}</small>
                                  </div>
                              </div>
