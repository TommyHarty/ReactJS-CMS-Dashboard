...
export default class EditProduct extends Component {
    ...
    componentDidMount(){
        ...
        axios.get(`http://localhost:8000/edit-product-api/${this.props.params.id}`)
        .then(response => {
            this.setState({
                id:response.data.id,
                page:response.data.page_id,
                ...
                additional_shipping:response.data.additional_shipping,
            });
        })
        .catch(function (error) {
            console.log(error);
    ...
    handleSubmit(){
        ...
        let uri = 'http://localhost:8000/update-product/'+this.props.params.id;
    ...
    handleDelete(object) {
        let uri = `http://localhost:8000/delete-product/${this.state.id}`;
        axios.delete(uri);
            browserHistory.push('/products');
    }
            ...
            <div className="row" style={contentStyle}>
                ...
                <div className="col-md-8">
                    <div className="text-right">
                        <Link to="/products"><FlatButton backgroundColor="#ebebeb" label="Cancel" style={{ marginRight:'15px', marginLeft:'15px' }} /></Link>
                        <FlatButton onClick={this.handleSubmit} className="button-bg"  label="Update Product" />
                        <div className="col-md-8">
                        </div>
                        <div className="col-md-4">
                            <Toggle
                    ...
                    <div className="text-right">
                        <FlatButton onClick={this.handleDuplicate} backgroundColor="#ebebeb" label="Duplicate Product" style={{ marginRight:'15px' }} />
                        <FlatButton onClick={this.handleDelete} backgroundColor="#ebebeb" label="Delete Product" />
                    </div>
                </div>
           </div>
        );
